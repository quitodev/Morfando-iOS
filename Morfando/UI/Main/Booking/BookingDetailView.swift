...
extension BookingDetailView {
    ...
    private var continueView: some View {
        VStack {
            Button {
                ...
                bookingData.tables = [tablesAvailable[0]]
                bookingData.tables_count = "1"
    ...
    private func getQRCode(bookingData: BookingBooking, bookingId: String) {
        ...
        if let imageData = uiImage.jpegData(compressionQuality: 0.50) {
            bookingViewModel.setBookings(bookingBooking: bookingData, bookingQr: imageData, userBookings: user.bookings)
        } else {
            //print("ERROR QR 3")
        }
    }
